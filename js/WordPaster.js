/*
	版权所有 2009-2012 武汉命运科技有限公司保留所有权利。
...
function WordPasterManager(mgr)
{
	...
	//已上传图片列表模块
	this.CompleteImageLister = {
		ImagesLocal: []
		, ImagesServer: []
		//检查是否存在本地相同图片
		, Exist: function(localPath)
		{
			for (var i = 0, l = this.ImagesLocal.length; i < l; ++i)
			{
				if (this.ImagesLocal[i] == localPath)
				{
					return true;
				}
			}
		...
		//添加到已完成列表
		, Add: function(localPath, serverPath)
		{
			this.ImagesLocal.push(localPath);
			this.ImagesServer.push(serverPath);
		}
		//清空已完成列表，关闭图片上传对话框时调用
		, Clear: function()
		{
			this.ImagesLocal.length = 0;
			this.ImagesServer.length = 0;
		}
		//获取已完成的图片地址
	...
	{
		for (regName in this.ImgRegexs)
		{
			var imgs = html.match(this.ImgRegexs[regName]);
			if (null != imgs)
			{
				return true;
			}
	...
	//添加Word图片
	this.AddWordImg = function(img)
	{
		var arr = this.ImgRegx.exec(img);
		arr = this.ImgAttr.alt.exec(img);
		var alt = RegExp.$1;
		arr = this.ImgAttr.width.exec(img);
		var width = RegExp.$1;
		arr = this.ImgAttr.height.exec(img);
		var height = RegExp.$1;
		arr = this.ImgAttr.src.exec(img);
		var src = RegExp.$1;

		this.AddFile(src, img, width, height);
	};
	...
	*/
	...
	this.MatchImage = function()
	{
		...
		for (regName in this.ImgRegexs)
		{
			var imgs = data.match(this.ImgRegexs[regName]);
			if (null != imgs)
			{
				var len = imgs.length;
				var url = "";
	...
	*/
	...
	this.CleanWord = function(data, bIgnoreFont, bRemoveStyles)
	{
		...
		// Remove "display:none" tags.bug:可能会将img标签也替换掉
...
WordPasterManager.prototype.AddFile = function(imgPath, imgHtml, width, height)
{
	...
	//if (!this.Exist(arguments[0]))
	{
		...
		var upFile = new WordFileUploader(fid, imgPath, this, width, height);
...
WordPasterManager.prototype.AddLocalFile = function(imgPath)
{
	...
	//if (!this.Exist(arguments[0]))
	{
		...
		var upFile = new WordFileUploader(fid, imgPath, this, 0, 0);
...
/*
	...
	属性：
		...
		InfDiv		上传信息层。
		Separator	分隔线
		Manager		WordPasterManager对象
*/
function WordFileUploader(fileID,filePath,mgr,width,height)
{
	var _this = this;
	_this.Manager = mgr;
	...
	_this.InsertHtml = mgr.InsertHtml;
	...
	_this.State = WordFileUploaderState.None;
	...
	_this.Ready = function()
	{
		...
		_this.State = WordFileUploaderState.Ready;
	};
	...
	_this.Post = function()
	{
		...
		_this.State = WordFileUploaderState.Posting;
		_this.ATL.Post();
	};
	...
	{
		//已上传列表中存在相同项
		if (this.Manager.CompleteImageLister.Exist(_this.LocalFile))
		{
			var url = _this.Manager.CompleteImageLister.Get(_this.LocalFile);
			_this.pButton.text("");
			_this.pProcess.css("width","100%");
			_this.pPercent.text("100%");
			_this.pMsg.text("上传完成");
			_this.State = WordFileUploaderState.Complete;
	...
	_this.PostNext = function()
	{
		if (_this.Manager.UnUploaderIdList.length > 0)
		{
			...
			//空闲状态
			if (WordFileUploaderState.Ready == obj.State)
			{
				obj.Post();
			}
		} //所有文件上传完毕
	...
	_this.Stop = function(fid)
	{
		...
		obj.ATL.Stop();
		obj.State = WordFileUploaderState.Stop;
		obj.pButton.text("重试");
	};
	...
	{
		var img = '<img src="../../../../WordPaster/js/' + imgSrc + '"/>';
...
function HttpUploader_Complete(obj)
{
	...
	obj.pMsg.text("上传完成");
	obj.State = WordFileUploaderState.Complete;

	//添加到已上传列表
	obj.Manager.CompleteImageLister.Add(obj.LocalFile, obj.ATL.GetResponse());
	//从上传列表中清除
	obj.Remove();
	//替换标签
...
function HttpUploader_Error(obj, err)
{
	...
	obj.pButton.text("重试");
	obj.State = WordFileUploaderState.Error;
...
function PasterManager()
{
	...
	_this.Config = {
		"EncodeType"		: "gb2312"
		, "Licensed"		: "武汉命运科技有限公司"
		, "PasteWordType"	: ""	//粘贴WORD的图片格式。JPG/PNG/GIF/BMP。推荐使用JPG格式，防止图片太大。
		...
		, "AppPath"			: ""
		, "CabPath"			: "http://www.ncmem.com/products/word-imagepaster/WordPaster.cab#version=1,4,64,65381"
		, "IcoPath"			: "http://www.ncmem.com/products/word-imagepaster/fckeditor2461/WordPaster/upload.gif"
		, "PostUrl"			: "http://localhost:81/wordpress/wp-content/plugins/wordpaster/upload.php"
		//x64
		...
		, "CabPath64"			: "http://www.ncmem.com/products/word-imagepaster/WordPaster64.cab#version=1,4,64,65381"
		//Firefox
		, "MimeType"	: "application/npWordPaster"
		, "XpiPath"		: "http://files.cnblogs.com/xproer/WordPaster.xpi"
		//Chrome
		, "CrxName"		: "chrWordPaster"
		, "MimeTypeChr"	: "application/chrWordPaster"
		, "CrxPath"		: "http://files.cnblogs.com/xproer/WordPaster.crx"
	};
	...
	_this.BrowserIE = {
		"GetHtml": function()
		{
			...
			*/
			var acx = '<div style="width:1px;height:1px;">';
			//Word图片粘贴
	...
	this.Load = function()
	{
		...
		*/
		...
		acx += '<img id="infIco" alt="进度图标" src="../../../../WordPaster/js/' + this.Config["IcoPath"] + 'upload.gif" /><span>图片上传中...</span><span>10%</span>';
		acx += '</div>';
	...
	this.CloseUploadFileDialog = function()
	{
		...
		this.CompleteImageLister.Clear(); //清空图片缓存
	};
	//打开粘贴图片对话框
	...
	//挂载粘贴事件
	_this.LoadPasteEvent = function(edt)
	{
